        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    sender.sendMessage(FrozedLib.INSTANCE.getNoPermissionMessage()); // TODO change
                    return true;
                }
                if (command.inGameOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(FrozedLib.INSTANCE.getOnlyGameCommandMessage());
                    return true;
                }
...
//            }
        ...
        try {
            ...
            FrozedLib.INSTANCE.getPlugin().getLogger().warning("[Commands] Error in register commands.yml");
        }
    }
    ...
    public void registerCommands(Object obj, List<String> aliases) {
        for (Method m : obj.getClass().getMethods()) {
            if (m.getAnnotation(Command.class) != null) {
                ...
                boolean parse = false;
                if (FrozedLib.INSTANCE.isExcludeCommands()) {
                    if (FrozedLib.INSTANCE.getExcludeCommandFile() != null && FrozedLib.INSTANCE.getExcludeCommandsPath() != null) {
                        for (String disabled : FrozedLib.INSTANCE.getExcludeCommandsList()) {
                ...
                if (parse) {
                    Bukkit.getConsoleSender().sendMessage(FrozedLib.INSTANCE.getDisableCommandMessage().replace("<command>", command.name()));
                    continue;
                }
